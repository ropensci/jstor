...
A complete pipeline for importing many files can be implemented like this:
...
specify it here.
- *.f*: the function to use (one of `find_metadata`, `find_authors`, `find_references`,
or `find_footnotes`)
...
jstor_import_wrapper(in_paths = vector_of_all_my_files, out_file = "metadata",
                     out_path = "path/to/out/dir", .f = find_metadata,
