...
from the zip archive, the second one works for file paths, so you need to unzip
the archive first.

I will first introduce `jst_import_zip()` and discuss the approach of with
`jst_import()` afterwards.
...
chapters, etc.). You need to manage the paths in order to apply the correct
functions to them.
...
what we acutally want to import: all of the data, or just parts? What kind of
...
In this case, we want to import data on articles (the standard metadata plus
...
the string you supplied to `out_file` to delianate the different types of output.
...
```{r}
jst_re_import(paste0(tmp, "/my_test_journal_article_jst_get_article-1.csv")) %>% 
  knitr::kable()
```

A side note on ngrams: For larger archives, importing all ngrams takes a very
...
You can find out mor about futures by reading the package vignette:
...
functions provided with the package doesn't make sense to your eyes, you could
want to look at the original files. For this, you could unzip the archive and
work with the files directly, which I will demonstrate in the following sections.
...
main difference being that it needs file paths as input.
