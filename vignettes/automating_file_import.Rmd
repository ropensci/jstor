...
utility functions implemented in `jstor`.
...
`parallel::mclapply` to speed up the process. Because of this, parallel processing
on windows is currently not implemented.
...
specify it here.
- *.f*: the function to use (one of `find_meta`, `find_authors`, `find_references`,
or `find_footnotes`)
- *files_per_batch*: how many files shoud be converted per batch? For my system,
20,000 files per batch worked well.
- *cores* number of cores to use for parallel processing (not on windows).

You could specify all parameters like this:
```{r, eval=FALSE}
jstor_import_wrapper(in_paths = vector_of_all_my_files, out_file = "metadata",
                     out_path = "path/to/out/dir", .f = find_meta,
