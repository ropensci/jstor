...
The function `jst_import()` takes care of errors and uses
...
`jst_import()` takes up to seven arguments:
...
specify it here.
- *.f*: the function to use (one of `jst_get_article`, `jst_get_authors`,
`jst_get_references`, `jst_get_footnotes`, `jst_get_book` or `jst_get_chapters`).
...
```{r, eval=FALSE}
jst_import(in_paths = vector_of_all_my_files, out_file = "metadata",
           out_path = "path/to/out/dir", .f = jst_get_article,
